...
namespace RAFS.Core.Services.Service
{
    public class PlantMaterialHistoryService : IPlantMaterialHistoryService
    {
        ...
        public List<ItemDTOHistory> GetInventoryItems(int farmid)
        {
            List<Item> items =  _uow.plantMaterialHistoryRepo.GetAll().Where(x => x.FarmId == farmid && x.Status == true &&(x.TypeId==10|| x.TypeId == 12 || x.TypeId == 18)).ToList();
           
            List<Unit> units = _uow.plantMaterialHistoryRepo.GetFarmUnit(farmid);
            List<ItemDTOHistory> itemDTOHistories = new List<ItemDTOHistory>();

            foreach (Item item in items) {
                ItemDTOHistory itemDTO = new ItemDTOHistory();
                itemDTO.Id = item.Id;
                itemDTO.ItemName = item.ItemName;
                itemDTO.TypeId = item.TypeId;
                itemDTO.UnitName = units.FirstOrDefault(x=>x.Id== item.UnitId).Name;
                itemDTOHistories.Add(itemDTO);
            }
            return itemDTOHistories;
        }
    }
}
