    ...
    setTimeout(function () {
        ...
        $.ajax({
            ...
            success: function (result, status, xhr) {
                ...
                $("#UnitName2").val(result["inventoryId"]);
            }
        });
        ...
        xhttp.onreadystatechange = function () {
            if (xhttp.readyState === 4) {
                ...
                if (xhttp.status === 200) {
                    ...
                    createToast('error', 'fa-solid fa-circle-exclamation', 'Error', "Số lượng tồn kho không đáp ứng");
                    pauseExecution(1000);
...
function validateInput(inputId) {
    ...
    if (inputId == "searchName") {
        if (value.length > 60) {
            document.getElementById("search-toast").innerHTML = 'Tên vật phẩm không được nhiều hơn 60 kí tự ';
...
function logSelectedValue() {
    ...
    const selectElement = document.getElementById("InventoryId");
    const selectedValue = selectElement.value;
    $("#UnitName").val(selectedValue);
...
var Manager = {
    ...
    GetAllItems: function (type) {
        ...
        if (flag == 0) {
            ...
            function onSuccess(jsonData) {
                obj = jsonData;
                var options = "<option value='0' selected>Chọn vật phẩm</option>";
                var optionsName = "<option value='0' selected> Đơn vị </option>";
                $.each(jsonData, function (i, item) {
                    if (item.typeId == 10) {
                    ...
                    optionsName += "<option value='" + item.id + "'>" + item.unitName + "</option>";
                });

                $('#UnitName').empty().append(optionsName);
                $('#UnitName2').empty().append(optionsName);
