...
function loadPlantMaterialHistory(plantid, type) {
    var element = document.getElementById("phan");
  
        element.disabled = true;
        element = document.getElementById("thuoc");
        element.disabled = true;
        element = document.getElementById("qua");
        element.disabled = true;
        element = document.getElementById("nhat");
        element.disabled = true;
        element = document.getElementById("selecttype");
        element.disabled = true;
    ...
    function onSuccess(jsonData) {
            ...
            if (indexof === -1) {
                ...
                if (type == 6 | type == 5 | type == 4) {
                    rows = "<tr class='view viessss '><td><h3> " + text + "</h3></td></tr>" +
                        ...
                        "<td colspan='7'>" +
                        "<div class='fold-content' style='overflow-y: auto; height: 600px;'>" +
                        "<h3>Chi tiết bản ghi</h3>" +
                        "<table class='small-friendly'>" +
                        "<thead>" +
                        "<tr>" +
                        "<th hidden><span class='visible-small' title='Id'>Id</span><span class='visible-big'>Id</span></th>" +
                        "<th><span class='visible-small' title='Tiêu đề'>Tiêu đề</span><span class='visible-big'>Tiêu đề</span></th>" +
                        "<th><span class='visible-small' title='Nội dung'>Nội dung</span><span class='visible-big'>Nội dung</span></th>" +
                        "<th><span class='visible-small' title='Ảnh'>Ảnh</span><span class='visible-big'>Ảnh</span></th>" +
                        "<th><span class='visible-small' title='Thời điểm cập nhật'>Thời điểm cập nhật</span><span class='visible-big'>Thời điểm cập nhật</span></th>" +

                        "</tr>" +
                        "</thead>" +
                        "<tbody id ='indexof" + listDate.length + "'>" +
                        "<tr>" +
                        "<td hidden data-th='Id'>" + item.id + "</td>" +
                        "<td data-th='Tiêu đề'>" + item.title + "</td>" +
                        "<td data-th='Nội dung'>" + item.body + "</td>" +
                        "<td data-th='Ảnh'> <img style='width:100%;     min-width: 100px;' src='" + item.image + "' alt='Girl in a jacket'></td>" +
                        "<td data-th='Thời điểm cập nhật'>" + texttime + "</td>" +
                        "</tr>" +
                        "</tbody>" +
                        "</table>" +
                        "</div>" +
                    ...
                    rows = "<tr class='view viessss ' ><td><h3> " + text + "</h3></td></tr>" +
                        ...
                        "<tr>" +
                        "<th hidden><span class='visible-small' title='Id'>Id</span><span class='visible-big'> Id</span></th>" +
                        "<th><span class='visible-small' title='Tên vật phẩm'>Tên vật phẩm</span><span class='visible-big'>Tên vật phẩm</span></th>" +
                        "<th><span class='visible-small' title='Số lượng'>Số lượng</span><span class='visible-big'>Số lượng </span></th>" +
                        "<th><span class='visible-small' title='Đơn vị'>Đơn vị</span><span class='visible-big'>Đơn vị </span></th>" +
                        "<th><span class='visible-small' title='Thời điểm cập nhật'>Thời điểm cập nhật</span><span class='visible-big'>Thời điểm cập nhật </span></th>" +
                        ...
                        "<tr>" +
                        "<td hidden data-th='Id'>" + item.id + "</td>" +
                        "<td data-th='Tên vật phẩm'>" + item.inventoryName + "</td>" +
                        "<td data-th='Số lượng'>" + item.quality + "</td>" +
                        "<td data-th='Đơn vị'>" + item.unit + "</td>" +
                        "<td data-th='Thời điểm cập nhật'>" + texttime + "</td>" +
                        "</tr>" +
                        "</tbody>" +
                        "</table>" +
        ...
        element = document.getElementById("phan");
...
function Update() {
    ...
    if (description.length > 180) {
        document.getElementById("description2-toast").innerHTML = 'Mô tả về cây không được nhiều hơn 180 kí tự ';
    ...
    if (name == null || name.length === 0) {
        ...
        document.getElementById("name2-toast").innerHTML = 'Tên của cây không được nhiều hơn 60 kí tự ';
    ...
    if (type === null || type.length === 0) {
        ...
        document.getElementById("type2-toast").innerHTML = 'Loại cây không được nhiều hơn 60 kí tự ';
    ...
    } else if (plantingMethod.length > 60) {
        document.getElementById("plantingMethod2-toast").innerHTML = 'Phương thức trồng của cây không được nhiều hơn 60 kí tự ';
    ...
    if (numofPassField == 5) {
        $.ajax({
            url: baseUrl + `Plant/CheckFarmAreaUpdate?farmid=${farmId}&area=${area}&unit=${document.getElementById("areaUnit2").value}&plantid=${document.getElementById("id2").value}`,
            method: 'GET',
            ...
            success: (data) => {
                ...
                if (data == true) {
                    ...
                    document.getElementById("area2-toast").innerHTML = 'Diện tích đã sử dụng vượt quá diện tích trang trại';
                }
            },
            error: function (xhr, status, error) {
                var errorMessage;
...
function validateInput(inputId) {
    ...
    if (inputId == "name2" || inputId == "name") {
        if (value == null || value.length === 0) {
            ...
            document.getElementById("name2-toast").innerHTML = 'Tên của cây không được nhiều hơn 60 kí tự ';
            document.getElementById("name-toast").innerHTML = 'Tên của cây không được nhiều hơn 60 kí tự ';
    ...
    if (inputId == "searchName" ) {
        if (value.length > 60) {
             document.getElementById("search-toast").innerHTML = 'Tên của cây không được nhiều hơn 60 kí tự ';

        } else {
             document.getElementById("search-toast").innerHTML = "";
        }
    }
    if (inputId == "typeFilter") {
        if (value.length > 60) {
            document.getElementById("typeFilter-toast").innerHTML = 'Phương pháp trồng không được nhiều hơn 60 kí tự ';

        } else {
            document.getElementById("typeFilter-toast").innerHTML = "";
        }
    }
    if (inputId == "description" || inputId == "description2") {
        if (value.length > 180) {
            document.getElementById("description-toast").innerHTML = 'Mô tả về cây không được nhiều hơn 180 kí tự ';
            document.getElementById("description2-toast").innerHTML = 'Mô tả về cây không được nhiều hơn 180 kí tự ';
        } else {
            document.getElementById("description-toast").innerHTML = '';
            document.getElementById("description2-toast").innerHTML = '';
        }
    }
    if (inputId == "type2" || inputId == "type") {
        if (value === null || value.length === 0) {
            ...
            document.getElementById("type2-toast").innerHTML = 'Loại cây không được nhiều hơn 60 kí tự ';
            document.getElementById("type-toast").innerHTML = 'Loại cây không được nhiều hơn 60 kí tự ';
    ...
    if (inputId == "plantingMethod2" || inputId == "plantingMethod") {
        if (value === null || value.length === 0) {
            ...
            document.getElementById("plantingMethod-toast").innerHTML = 'Phương thức trồng của cây không được nhiều hơn 60 kí tự ';
            document.getElementById("plantingMethod2-toast").innerHTML = 'Phương thức trồng của cây không được nhiều hơn 60 kí tự ';
...
function Create() {
   ...
   //     createToast('warning', 'fa-solid fa-triangle-exclamation', 'Warning', 'Tên của cây không được để trống');
    ...
    if (numofPassField == 6) { 
         ...
         $.ajax({
             url: baseUrl + `Plant/CheckFarmArea?farmid=${farmId}&area=${area}&unit=${document.getElementById("areaUnit").value}`,
            method: 'GET',
            contentType: 'application/json',
             success: (data) => {
                 ...
                 if (data == true) {
                     ...
                     document.getElementById("area-toast").innerHTML = 'Diện tích đã sử dụng vượt quá diện tích trang trại';
                 }
             },
             error: function (xhr, status, error) {
                 var errorMessage;
    ...
    var newColumnHTML = `
            ...
            <div class="card bg-light">
                <div class="card-body">
                       ...
                       <button id="milestoneUpdateTime${id}" type="button" style="padding-bottom: 8px;padding-top: 8px;background-color: darkcyan; max-height: 60px;" class="btn btn-primary update-time">Đưa lên đầu </button>
...
function printQRCode() {
    ...
    var printWindow = window.open("", "Print QR Code");
    printWindow.document.write("<html><head><title>Print QR Code</title>");
    // Add style for printing
    printWindow.document.write("<style>@media print { img { width: 100%; } }</style>");
        ...
        setTimeout(function () {
            ...
            function onSuccess2(jsonData) {
                ...
                $.each(jsonData, function (i, item) {
                    ...
                    <div class="card-body p-2">
                        <div class="card-title">
                            ...
                            <h3 href="#" style="color:black;" class="lead font-weight-light">${item.name}</h3>
                        </div>
                            <p>
                ...
                $(".update-time").on("click", function (e) {
                    var resId = $(this).attr("id"); // Get the id attribute of the clicked button
                    resId = resId.slice(19); // Remove the first 20 characters
                    resId = resId.replace(/\D/g, ''); // Remove any non-digit characters
                    resId = parseInt(resId); // Convert to an integer

                    var xhttp = new XMLHttpRequest();

                    xhttp.open("PUT", baseUrl + "Plant/UpdateMilestoneTime?milestoneid="+resId, true);
                    xhttp.setRequestHeader("Content-type", "application/json");
                    var obj = {
