...
namespace RAFS.Web.ApiControllers
{
    ...
    public class PlantMaterialHistoryController : ControllerBase
    {
        ...
        public ActionResult<IEnumerable<ItemDTOHistory>> GetInventoryItem(int farmid)
        {
            try
            {
        ...
        public ActionResult<IEnumerable<TakeAndSendMaterial>> GetAllPlantMaterialHistory(int plantid)
        {
            try
            {
                var milestoneList = _plantMaterialHistoryService.GetAllPlantMaterialHistory(plantid).OrderByDescending(x => x.Id).ToList();
                return Ok(milestoneList);
            }
            catch (Exception ex)
            {
        ...
        public ActionResult<IEnumerable<TakeAndSendMaterial>> GetAllFarmMaterialHistory(int farmid)
        {
            try
            {
                var milestoneList = _plantMaterialHistoryService.GetAllFarmMaterialHistory(farmid).OrderByDescending(x => x.Id).ToList();
                return Ok(milestoneList);
            }
            catch (Exception ex)
            {
        ...
        public ActionResult<IEnumerable<TakeAndSendMaterialDTO>> GetAllPlantMaterialHistoryByType(int plantid,int typeId)
        {
            try
            {
                var milestoneList = _plantMaterialHistoryService.GetAllPlantMaterialHistoryByType(plantid, typeId).OrderByDescending(x => x.Id).ToList();
                return Ok(milestoneList);
            }
            catch (Exception ex)
            {
