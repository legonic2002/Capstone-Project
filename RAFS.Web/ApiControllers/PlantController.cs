...
namespace RAFS.Web.ApiControllers
{
    ...
    public class PlantController : ControllerBase
    {
        ...
        [Authorize(Roles = "Technician")]
        [HttpGet("CheckFarmArea")]
        public ActionResult<bool> CheckFarmArea(int farmid, int area, string unit)
        {
            try
            {
              

                var milestone = _plantService.CheckArea(farmid,area,unit);
        ...
        [Authorize(Roles = "Technician")]
        [HttpGet("CheckFarmAreaUpdate")]
        public ActionResult<bool> CheckFarmAreaUpdate(int farmid, int area, string unit,int plantid)
        {
            try
            {


                var milestone = _plantService.CheckAreaUpdate(farmid, area, unit, plantid);
        ...
        public ActionResult<IEnumerable<Milestone>> GetmilestoneList(int farmid)
        {
            try
            {
                var milestoneList = _milestoneService.GetAllMilestone(farmid).OrderByDescending(x=>x.LastUpdate).ToList();
                return Ok(milestoneList);
            }
            catch (Exception ex)
            {
        ...
        [Authorize(Roles = "Technician")]
        [HttpPut("UpdateMilestoneTime")]
        public ActionResult<Milestone> UpdateMilestoneTime(int milestoneid)
        {
            try
            {
                Milestone milestone1 = _milestoneService.GetMilestoneById(milestoneid);

                milestone1.LastUpdate = DateTime.Now;
              

                var milestoneList = _milestoneService.UpdateMilestoneTime(milestone1);
                if (milestoneList)
                {
                    return Ok(milestoneList);
                }
