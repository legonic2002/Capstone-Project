...
namespace RAFS.Web.ApiControllers
{
    ...
    public class DiaryController : ControllerBase
    {
        ...
        public ActionResult<IEnumerable<DiaryDTO>> GetDiaryList(int farmid)
        {
            try
            {
                var milestoneList = _diaryService.GetAllDiary(farmid).OrderByDescending(x=>x.Id).ToList();
        ...
        public ActionResult<IEnumerable<DiaryDTO>> GetPlantDiaryList(int plantid)
        {
            try
            {
                var milestoneList = _diaryService.GetAllPlantDiary(plantid).OrderByDescending(x => x.Id).ToList();
                return Ok(milestoneList);
            }
            catch (Exception ex)
            {
        ...
        public ActionResult<IEnumerable<DiaryDTO>> GetPlantDiaryListByType(int plantid, int type)
        {
            try
            {
                var milestoneList = _diaryService.GetAllPlantDiaryByType(plantid,type).OrderByDescending(x => x.Id).ToList();
                return Ok(milestoneList);
            }
            catch (Exception ex)
            {
