...
namespace RAFS.Web.Controllers
{
    ...
    public class AuthenticationController : Controller
    {
        ...
        public IActionResult Login()
        {
            ...
            if (TempData.ContainsKey("accountDisabled"))
            {
                ViewBag.accountDisabled = TempData["accountDisabled"];
            }

            if (TempData.ContainsKey("farmDisabled"))
            {
                ViewBag.farmDisabled = TempData["farmDisabled"];
            }

            if (TempData.ContainsKey("farmNotAvailable"))
            {
                ViewBag.farmNotAvailable = TempData["farmNotAvailable"];
            }
        ...
        public async Task<IActionResult> Login(UserLoginDTO user)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                AspUser currentUser = await _userManager.FindByNameAsync(user.UserName);
        ...
        public async Task<IActionResult> ForgotPassword(ForgotPasswordModel rpvm)
        {
            ...
            if (user == null)
            {
                ModelState.AddModelError("emailNotFound", "Không tìm thấy tài khoản nào với địa chỉ email này");
                return View(rpvm);
            }
            ModelState.AddModelError("confirm", "Vui lòng kiểm tra Email của bạn để tiếp tục.");
        ...
        public async Task<IActionResult> GoogleLoginCallback(string returnUrl = null, string remoteError = null)
        {
            ...
            if (user != null)
            {
                if (!user.Status)
                {
                    TempData["accountDisabled"] = "Tài khoản đã bị vô hiệu hoá.";
                    return RedirectToAction("Login", "Authentication");
                }
            }
                ...
                foreach (var role in userRoles)
                {
                    if (role.Equals("Technician"))
                    {
                        ...
                        if (farm != null)
                        {
                            ...
                            {
                                await _signInManager.SignOutAsync();
                                TempData["farmDisabled"] = "Trang trại chủ quản không hoạt động.";
                                return RedirectToAction("Login", "Authentication");
                            }
                        ...
                        if (farm == null)
                        {
                            ...
                            await _signInManager.SignOutAsync();
                            TempData["farmNotAvailable"] = "Tài khoản nhân viên không thuộc vào trang trại nào.";
                            return RedirectToAction("Login", "Authentication");
                        }
